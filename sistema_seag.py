...
if verificar_senha():
    ...
    # Conexão com Google Sheets (Link configurado via Secrets no Streamlit Cloud)
    ...
    try:
        ...
        if df.empty or len(df.columns) < 2:
            ...
            conn.update(data=df)
            st.info("Planilha inicializada com os dados do setor!")
        df = df.fillna('')
    except:
        df = criar_dados_iniciais()
        st.warning("Rodando em modo offline. Verifique a configuração da planilha.")
    ...
    def gerar_pdf_custom(dataframe, filtro_coluna, filtro_valor, titulo_relatorio):
        ...
        pdf.cell(190, 7, f"Relatório gerado em: {datetime.now().strftime('%d/%m/%Y às %H:%M')}", ln=True, align='R')
        pdf.ln(5)
    ...
    for nome, col, val in rel_btns:
        if st.sidebar.button(f"📄 Relatório: {nome}"):
            pdf_bytes = gerar_pdf_custom(df, col, val, f"Entidades {nome}")
            st.sidebar.download_button(f"⬇️ Baixar {nome}", pdf_bytes, f"SEAG_{nome}.pdf", "application/pdf")
